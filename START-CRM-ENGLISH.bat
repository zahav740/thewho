...
if !errorlevel!==0 (
    ...
    echo ⏳ Backend starting... (attempt %attempts%/15)
    goto :check_backend
)
...
REM ⚡ ИСПРАВЛЕНИЕ: Установка переменных для автоматического открытия браузера
echo Setting up auto-browser environment...
set BROWSER=default
set OPEN_BROWSER=true

REM ⚡ ИСПРАВЛЕНИЕ: Запуск только одного процесса npm start с правильными переменными
