  ...
  private mapToRecommendedOrders(operations: Operation[]): RecommendedOrderDto[] {
    return operations.map((operation) => {
      ...
      if (!operation.order) {
        ...
        return {
          orderId: 0, // Исправлено: number вместо string
          ...
          priority: 4 as Priority, // Приведение к Priority enum
          deadline: new Date(),
          pdfUrl: null,
          previewUrl: null,
        };
      ...
      return {
        ...
        operationType: operation.operationType || 'unknown',
        estimatedTime: operation.estimatedTime || 0,
        priority: (operation.order.priority as Priority) || Priority.LOW, // Приведение к Priority enum
