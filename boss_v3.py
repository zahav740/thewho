...
def create_monitoring_system():
    ...
    ws_dashboard['B4'] = f'=SUMIF(MainData!B:B,"<0.7")'
    ws_dashboard['B4'].font = Font(bold=True, color=CRITICAL_COLOR)
    ws_dashboard['A5'] = "Общий OEE:"
    ws_dashboard['B5'] = f'=IFERROR(AVERAGE(MainData!B:B),0)'
    ws_dashboard['B5'].number_format = '0.0%'
    ws_dashboard['A6'] = "Общий KPI:"
    ws_dashboard['B6'] = f'=IFERROR(AVERAGE(MainData!C:C),0)'
    ws_dashboard['B6'].number_format = '0.0%'
    ws_dashboard['A7'] = "Лучший оператор:"
    ws_dashboard['B7'] = "N/A"  # Требует доработки
    ws_dashboard['A8'] = "Лучший станок:"
    ws_dashboard['B8'] = f'=INDEX(MainData!A:A,MATCH(MAX(MainData!B:B),MainData!B:B,0))'
    ...
    for row, machine in enumerate(MACHINES, 2):
        ...
        ws_gantt.cell(row=row, column=2).value = datetime(2025, 6, 28, 22, 30).strftime('%d.%m.%Y')  # 10:30 PM IDT
    ...
    # --- 4. Сохранение файла ---
    filename = f"OEE_Monitoring_System_v13.9_{datetime(2025, 6, 28, 22, 30).strftime('%Y%m%d_%H%M')}.xlsx"
