...
def create_monitoring_system():
    ...
    column_widths = {
        ...
        'MainData': {openpyxl.utils.get_column_letter(i): 18 for i in range(1, 6)},
    ...
    translations_data = [
        ...
        ["N-чертежа", "Drawing Number", "Номер чертежа детали"],
    ]
    ...
    for row, machine in enumerate(MACHINES, 2):
        ...
        ws_main_data.cell(row=row, column=4).value = f'=SUM(Station_{machine.split(" ")[0]}!J:J)'  # Good_pcs
        ws_main_data.cell(row=row, column=5).value = f'=SUM(Station_{machine.split(" ")[0]}!I:I)'  # Defect_pcs
        for col_idx in range(1, len(headers_main) + 1):
    ...
    for row, machine in enumerate(MACHINES, 2):
        ...
        ws_summary.cell(row=row, column=3).value = f'=IFERROR(AVERAGE(Station_{machine.split(" ")[0]}!N:N),0)'  # OEE
        ws_summary.cell(row=row, column=3).number_format = '0.0%'
        ws_summary.cell(row=row, column=4).value = f'=IFERROR(AVERAGE(Station_{machine.split(" ")[0]}!Q:Q),0)'  # KPI
    ...
    for machine in MACHINES:
        ...
        headers_station = [
            "Date", "N-чертежа", "Operator", "Shift_min", "Setup_min", "Downtime_min", "Planned_pcs",
        ...
        # Валидация данных
        dv_date = DataValidation(type="date", operator="between", formula1="2025-01-01", formula2="2025-12-31")
        dv_date.add("A3:A1002")
        ws.add_data_validation(dv_date)
        for row in range(3, 1003):  # Форматирование и комментарий
            cell = ws.cell(row=row, column=1)
            cell.number_format = "dd.mm.yyyy"  # Явное форматирование даты
            cell.comment = Comment("Введите дату в формате дд.мм.гггг. Для календаря: 1) Выделите колонку A, 2) 'Данные' -> 'Проверка данных' -> 'Дата', 3) Дважды кликните (работает в Excel 365).", "Grok")

        dv_drawing = DataValidation(type="textLength", operator="lessThanOrEqual", formula1="20")
        dv_drawing.add("B3:B1002")
        ws.add_data_validation(dv_drawing)
        ...
        dv_shift = DataValidation(type="whole", operator="between", formula1="0", formula2="1440")
        ...
        dv_downtime = DataValidation(type="whole", operator="between", formula1="0", formula2="1440")
        ...
        dv_planned.add("G3:G1002")
        ws.add_data_validation(dv_planned)
        dv_actual = DataValidation(type="whole", operator="between", formula1="0", formula2="1000")
        dv_actual.add("H3:H1002")
        ws.add_data_validation(dv_actual)
        dv_defect = DataValidation(type="custom", formula1="AND(I3>=0,I3<=H3)")
        dv_defect.add("I3:I1002")
        ws.add_data_validation(dv_defect)

        # Заполнение данных и формул с динамическим shift_min
        for row in range(3, 1002):
            for col in range(1, len(headers_station) + 1):
                ws.cell(row=row, column=col).border = THIN_BORDER
                ws.cell(row=row, column=col).fill = TABLE_BG_FILL
            ws.cell(row=row, column=4).value = 480  # Начальное значение shift_min (8 часов)
            ws.cell(row=row, column=10).value = f'=IFERROR(H{row}-I{row},0)'  # Good_pcs
            ws.cell(row=row, column=11).value = f'=IFERROR((D{row}-F{row})/D{row},0)'  # Availability_pct
            ws.cell(row=row, column=11).number_format = '0.0%'
            ws.cell(row=row, column=12).value = f'=IFERROR(H{row}/G{row},0)'  # Performance_pct
            ws.cell(row=row, column=12).number_format = '0.0%'
            ws.cell(row=row, column=13).value = f'=IFERROR(J{row}/H{row},0)'  # Quality_pct
            ws.cell(row=row, column=13).number_format = '0.0%'
            ws.cell(row=row, column=14).value = f'=IFERROR(K{row}*L{row}*M{row},0)'  # OEE_pct
            ws.cell(row=row, column=14).number_format = '0.0%'
            ws.cell(row=row, column=15).value = f'=IFERROR(E{row}/D{row},0)'  # Setup_ratio_pct
            ws.cell(row=row, column=15).number_format = '0.0%'
            ws.cell(row=row, column=16).value = f'=M{row}'  # Setup_quality_pct
            ws.cell(row=row, column=16).number_format = '0.0%'
            ws.cell(row=row, column=17).value = f'=IFERROR(N{row}*0.4+(1-O{row})*0.25+P{row}*0.2+K{row}*0.15,0)'  # KPI_pct
    ...
    ws_dashboard['A7'] = "Общий OEE:"  # Увеличен интервал до A7
    ws_dashboard['B7'] = f'=IFERROR(AVERAGE(MainData!B:B),0)'
    ws_dashboard['B7'].number_format = '0.0%'

    ws_dashboard['A10'] = "Общий KPI:"  # Увеличен интервал до A10
    ws_dashboard['B10'] = f'=IFERROR(AVERAGE(MainData!C:C),0)'
    ws_dashboard['B10'].number_format = '0.0%'

    ws_dashboard['A13'] = "Лучший оператор:"  # Увеличен интервал до A13
    ws_dashboard['B13'] = "N/A"

    ws_dashboard['A16'] = "Лучший станок:"  # Увеличен интервал до A16
    ws_dashboard['B16'] = f'=INDEX(MainData!A:A,MATCH(MAX(MainData!B:B),MainData!B:B,0))'
    ...
    chart.height = 15  # Установка высоты графика
    chart.width = 30   # Установка ширины графика
    ws_dashboard.add_chart(chart, "A20")  # Перемещен ниже к A20 для большего отступа
    ...
    for row, machine in enumerate(MACHINES, 2):
        ...
        ws_gantt.cell(row=row, column=2).value = datetime(2025, 6, 28, 23, 15).strftime('%d.%m.%Y')  # 11:15 PM IDT
    ...
    for row, machine in enumerate(MACHINES, 2):
        ...
        ws_results.cell(row=row, column=2).value = f'=SUM(Station_{machine.split(" ")[0]}!J:J)'
        ws_results.cell(row=row, column=3).value = f'=SUM(Station_{machine.split(" ")[0]}!I:I)'
        ws_results.cell(row=row, column=4).value = f'=IFERROR(AVERAGE(Station_{machine.split(" ")[0]}!N:N),0)'
        ws_results.cell(row=row, column=4).number_format = '0.0%'
        ws_results.cell(row=row, column=5).value = f'=IFERROR(AVERAGE(Station_{machine.split(" ")[0]}!Q:Q),0)'
    ...
    # --- 4. Сохранение файла ---
    filename = f"OEE_Monitoring_System_v13.9_{datetime(2025, 6, 28, 23, 15).strftime('%Y%m%d_%H%M')}.xlsx"
