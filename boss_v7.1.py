...
# -*- coding: utf-8 -*-
"""
СИСТЕМА МОНИТОРИНГА OEE/KPI v7.1 - УЛУЧШЕННАЯ ВАЛИДАЦИЯ ДАТ
Поддержка множественных форматов дат для удобства пользователей
"""
...
def create_monitoring_system():
    ...
    """
    print("🚀 Запуск создания УЛУЧШЕННОЙ системы мониторинга v7.1...")
    ...
    translations_data = [
        ...
        ["Date Formats", "Форматы дат", "29.06.2025 or 6/29/2025 or 29-06-2025"],
    ]
    ...
    # ============ Листы по станкам (УЛУЧШЕННАЯ ВАЛИДАЦИЯ ДАТ) ============
    ...
    for machine in MACHINES:
        ...
        # Убираем строгую валидацию, оставляем только базовую проверку года
        dv_date = DataValidation(
            ...
            promptTitle="📅 Ввод даты / Date Input",
            prompt="ПОДДЕРЖИВАЕМЫЕ ФОРМАТЫ / SUPPORTED FORMATS:\n\n✅ 29.06.2025 (русский / Russian)\n✅ 6/29/2025 (американский / US)\n✅ 29-06-2025 (альтернативный / Alternative)\n✅ 2025-06-29 (ISO стандарт / ISO)\n\n💡 БЫСТРЫЙ ВВОД / QUICK INPUT:\n• Ctrl+; = сегодняшняя дата / today's date\n• Excel 365: двойной клик для календаря / double-click for calendar\n\n⚠️ ГОД ДОЛЖЕН БЫТЬ 2020-2030 / YEAR MUST BE 2020-2030",
            showErrorMessage=True,
            errorTitle="⚠️ Проверьте дату / Check Date",
            error="Убедитесь что год в диапазоне 2020-2030. Excel принимает разные форматы дат. / Make sure year is 2020-2030. Excel accepts various date formats."
        )
        ...
        for row in range(3, 1002):
            ...
            # Форматирование даты - поддержка разных форматов
            ws.cell(row=row, column=1).number_format = "DD.MM.YYYY"
            ws.cell(row=row, column=1).comment = Comment(
                "📅 ПОДДЕРЖИВАЕМЫЕ ФОРМАТЫ ДАТ / SUPPORTED DATE FORMATS:\n"
                "═══════════════════════════════════════════════════════\n"
                "✅ 29.06.2025 (русский формат / Russian format)\n"
                "✅ 6/29/2025 (американский формат / US format)\n" 
                "✅ 29-06-2025 (альтернативный / Alternative)\n"
                "✅ 2025-06-29 (ISO стандарт / ISO standard)\n"
                "\n"
                "💡 БЫСТРЫЕ СПОСОБЫ / QUICK METHODS:\n"
                "• Ctrl+; = вставить сегодня / insert today\n"
                "• Excel 365: двойной клик = календарь / double-click = calendar\n"
                "• Просто печатайте / Just type - Excel умный!\n"
                "\n"
                "🎯 Excel автоматически распознает формат!\n"
                "🎯 Excel automatically recognizes the format!", 
                "OEE Monitoring System v7.1"
            )
    ...
    # --- 4. Сохранение файла ---
    filename = f"OEE_Monitoring_v7.1_FlexibleDates_{datetime(2025, 6, 28, 23, 15).strftime('%Y%m%d_%H%M')}.xlsx"
    filepath = os.path.join(os.getcwd(), filename)
    try:
        wb.save(filepath)
        print(f"✅ УСПЕХ! Файл создан: {filename}")
        print("\n🎯 УЛУЧШЕНИЯ v7.1:")
        print("   ✓ Поддержка множественных форматов дат")
        print("   ✓ 29.06.2025 (русский)")
        print("   ✓ 6/29/2025 (американский)")
        print("   ✓ 29-06-2025 (альтернативный)")
        print("   ✓ 2025-06-29 (ISO)")
        print("   ✓ Расширенная валидация (2020-2030)")
        print("   ✓ Улучшенные подсказки")
        print("   ✓ Автоматическое распознавание Excel")
        print("\n📊 БЕНЧМАРКИ OEE (без изменений):")
