...
# Проверяем, работает ли API health endpoint
curl -s http://localhost:5100/api/health > /dev/null
if [ $? -eq 0 ]; then
    echo "✅ Backend запущен и работает на порту 5100"
    
    echo "📋 Проверяем shifts endpoint..."
    echo "GET /api/shifts:"
    curl -s -o /dev/null -w "%{http_code}" http://localhost:5100/api/shifts
    echo ""
...
curl -s http://localhost:5101 > /dev/null
if [ $? -eq 0 ]; then
