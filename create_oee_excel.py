...
def create_oee_kpi_excel_file():
    ...
    # Формулы для автоматических расчетов (строка 2) - ИСПРАВЛЕННЫЕ
    formulas = {
        'K2': '=I2-J2',  # Годные детали
        'L2': '=IF(D2>0,(E2+F2)/D2*100,0)',  # Доступность % = (Наладка + Производство) / Смена
        'M2': '=IF(H2>0,I2/H2*100,0)',  # Производительность %
        'N2': '=IF(I2>0,K2/I2*100,0)',  # Качество %
        'O2': '=L2*M2*N2/10000',  # OEE станка %
        'P2': '=IF(D2>0,E2/D2*100,0)',  # Доля наладки %
        'Q2': '=IF(F2>0,I2/(F2/25)*0.6+N2*0.4,0)',  # KPI оператора (БЕЗ штрафа за наладку)
        'R2': '=IF(D2>0,(G2/D2)*100,0)'  # Простои %
    }
    ...
    # Копируем формулы на 100 строк - ИСПРАВЛЕННЫЕ
