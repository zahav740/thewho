...
# -*- coding: utf-8 -*-
"""
Генератор Excel файла OEE/KPI с русскими названиями
...
def create_oee_kpi_excel_russian():
    """
    Создает полный Excel файл с системой OEE и KPI на русском языке
    """
    print("🚀 Создаем Excel файл OEE/KPI с русскими названиями...")
    ...
    ws_input['A18'] = "ПРИМЕЧАНИЯ:"
    ws_input['A18'].font = Font(bold=True, color="FF0000")
    ws_input['A19'] = "• Смена всегда 480 минут (8 часов)"
    ws_input['A20'] = "• Наладка + Производство + Простои = Смена"
    ws_input['A21'] = "• Годные = Факт - Брак"
    ws_input['A22'] = "• Все времена указывать в минутах"
    ...
    # ============ ЛИСТ 4: ПАНЕЛЬ УПРАВЛЕНИЯ ============
    print("🎛️ Создаем лист 'Панель_управления'...")
    ws_dash = wb.create_sheet("Панель_управления")
    ...
    settings_data = [
        ...
        ("A19", "🔧 ЦВЕТОВЫЕ ИНДИКАТОРЫ:"),
        ("A20", "🟢 Зеленый: OEE ≥80%, KPI ≥85% - Отличный результат"),
        ("A21", "🟡 Желтый: OEE 70-79%, KPI 75-84% - Хороший результат"),
        ("A22", "🔴 Красный: OEE <70%, KPI <75% - Требует улучшения"),
    ]
    ...
    ws_settings.column_dimensions['E'].width = 35
    ...
    try:
        ...
        print("   🔸 Лист 'Панель_управления' - ключевые показатели в реальном времени")
...
def main():
    """Основная функция"""
    print("=" * 70)
    print("🏭 ГЕНЕРАТОР EXCEL ФАЙЛА OEE/KPI ДЛЯ ПРОИЗВОДСТВА (РУССКАЯ ВЕРСИЯ)")
    print("=" * 70)
    ...
    # Создаем файл
    filepath = create_oee_kpi_excel_russian()
    ...
    if filepath:
        ...
        print(f"   2. Перейдите на лист 'Ввод_данных'")
        print(f"   3. Заполните форму реальными данными")
        print(f"   4. Проверьте автоматические расчеты на листе 'Данные'")
        print(f"   5. Просмотрите 'Панель_управления' для общей картины")
    else:
