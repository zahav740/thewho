/**
 * @file: OrderForm.SIMPLE.tsx - ИСПРАВЛЕННАЯ ВЕРСИЯ БЕЗ ОШИБОК TYPESCRIPT
 * @description: Упрощенная форма с исправленной поддержкой PDF и без ошибок TypeScript
 * @updated: 2025-07-08 - Исправлены все TypeScript ошибки
 */
...
import { PlusOutlined, DeleteOutlined, FilePdfOutlined, UploadOutlined, ExpandOutlined } from '@ant-design/icons';
...
export const OrderForm: React.FC<OrderFormProps> = ({
  ...
  console.log('🔧 OrderForm (ИСПРАВЛЕННАЯ) rendered:', { visible, orderId, isEdit });
    ...
    onSuccess: async (newOrder) => {
      ...
      if (pdfFile && newOrder.id) {
        try {
          ...
          // ИСПРАВЛЕНО: Правильное получение данных формы
          const formData = getValues();
          const drawingNumber = formData.drawingNumber || `order_${newOrder.id}`;
          await pdfApiFixed.uploadPdf(newOrder.id, drawingNumber, pdfFile);
    ...
    onSuccess: async () => {
      ...
      if (pdfFile && orderId) {
        try {
          ...
          await pdfApiFixed.uploadPdf(orderId, drawingNumber, pdfFile);
  ...
  const handlePdfRemove = async () => {
    ...
    try {
      ...
      await pdfApiFixed.deletePdf(orderId);
  ...
  const getCurrentPdfUrl = () => {
    if (currentPdfPath) {
      // ИСПРАВЛЕНО: Правильное получение URL для PDF
      const formData = getValues();
      const drawingNumber = formData.drawingNumber || 'unknown';
      return pdfApiFixed.getPdfUrl(drawingNumber, currentPdfPath);
    }
    return '';
  };
