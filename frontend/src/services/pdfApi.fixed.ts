/**
 * @file: pdfApi.fixed.ts
 * @description: API для работы с PDF файлами - исправленная версия с правильными экспортами
 * @dependencies: api
 * @created: 2025-07-08
 * @updated: 2025-07-08 - Исправление экспортов для TypeScript
 */
import api from './api';
...
export interface PdfDuplicateConflict {
  error: string;
  message: string;
  duplicateInfo: PdfDuplicateCheck;
  actions: Array<{
    key: string;
    label: string;
    description: string;
...
export interface PdfUploadResponse {
  ...
  error?: string;
  conflict?: PdfDuplicateConflict;
}

const pdfApiImplementation = {
  /**
   ...
   */
  deletePdf: async (orderId: number, archive: boolean = false): Promise<void> => {
    console.log('🗑️ Удаление PDF для заказа:', orderId, archive ? '(архивирование)' : '');
    try {
      const params = archive ? '?archive=true' : '';
      await api.delete(`/pdf-enhanced/orders/${orderId}${params}`);
  ...
  useExistingPdf: async (orderId: number, existingFilename: string, drawingNumber: string): Promise<PdfUploadResult> => {
    ...
    try {
      ...
      return await pdfApiImplementation.uploadPdf(orderId, drawingNumber, emptyFile, { useExisting: true });
    } catch (error: any) {
  ...
  createRevision: async (orderId: number, drawingNumber: string, file: File): Promise<PdfUploadResult> => {
    ...
    try {
      return await pdfApiImplementation.uploadPdf(orderId, drawingNumber, file, { createRevision: true });
    } catch (error: any) {
  ...
  replacePdf: async (orderId: number, drawingNumber: string, file: File): Promise<PdfUploadResult> => {
    ...
    try {
      return await pdfApiImplementation.uploadPdf(orderId, drawingNumber, file, { replaceDuplicate: true });
    } catch (error: any) {
...
// Экспорты для разных способов импорта
export const pdfApiFixed = pdfApiImplementation;
export const pdfApi = pdfApiImplementation;
export default pdfApiImplementation;
